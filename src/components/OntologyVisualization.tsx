...
const CARD_HEIGHT = 182; // As per CSS
const CHILD_X_SPACING = 364; // childWidth from original script
const CHILD_Y_SPACING = 260; // childY offset from original script
...
const OntologyVisualization: React.FC = () => {
  ...
  const centerOnNodes = useCallback((targetNodes: OntologyNode[], newScale?: number) => {
    ...
    const padding = targetNodes.length > 1 ? 150 : 50; // Reduced padding for single node centering
    minX -= padding;
    minY -= padding;
    maxX += padding;
    maxY += padding;
    ...
    const calculatedScale = newScale ?? Math.min(Math.min(viewportWidth / areaWidth, viewportHeight / areaHeight) * 0.8, 1.5);
  ...
  useEffect(() => {
    ...
    fetch(dataUrl)
      ...
      .then((data: OntologyDataNode) => {
        ...
        const initialX = visualizationContainerRef.current ? visualizationContainerRef.current.offsetWidth / 2 - CARD_WIDTH / 2 : 1400; 
        const initialY = visualizationContainerRef.current ? visualizationContainerRef.current.offsetHeight / 3 - CARD_HEIGHT / 2 : 160; 
        const processedNodes = processNodeRecursive(data, initialX, initialY, 0, null, true);
        console.log("Processed nodes (first 5):", processedNodes.slice(0,5)); // DEBUG
        setNodes(processedNodes);
        if (processedNodes.length > 0 && processedNodes[0].isRoot) {
           // Let centerOnNodes calculate the scale initially by not passing a scale argument
           setTimeout(() => {
            console.log("Initial centering on root:", processedNodes[0]); // DEBUG
            centerOnNodes([processedNodes[0]]); 
           }, 100); 
        }
      })
      ...
      if (isEndNode) {
        if (currentlyZoomedNodeId === clickedNodeId) {
          ...
          if (viewportRef.current) {
            ...
            const targetWidth = viewportRect.width * 0.20;
            const newScale = Math.min(targetWidth / CARD_WIDTH, 1.5);
    ...
    >
      ...
      >
        {nodes.filter(node => node.visible).map(node => (
          <div
            key={node.id}
            className={`card ${node.isRoot ? 'root-card' : ''} ${node.collapsed ? 'collapsed' : ''}`}
          ...
          >
            <div className="card-definition">{node.definition}</div>
